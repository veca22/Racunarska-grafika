...
namespace AssimpSample
{
    ...
    public class AssimpScene : IDisposable
    {
        ...
        private uint[] textIds;

        //Mapiranje teksture na njen identifikator
        private Dictionary<TextureSlot, uint> textureMappings;
        ...
        public AssimpScene(String scenePath, String sceneFileName, OpenGL gl)
        {
            ...
            this.textureMappings = new Dictionary<TextureSlot, uint>();
            this.gl = gl;
        }
            ...
            if (node.HasMeshes)
            {
                foreach (int meshIndex in node.MeshIndices)
                {
                    ...
                    if (material.GetAllTextures().Length > 0)
                        gl.BindTexture(OpenGL.GL_TEXTURE_2D, textureMappings[material.GetAllTextures()[0]]);
        ...
        /// Ucitavanje tekstura
        /// </summary>
        /// <param name="textureSlot">Informacije o teskturi koje se ucitavaju</param>
        private void loadTextures()
        {
            ...
            textIds = new uint[texCount];
            gl.GenTextures(texCount, textIds);
            ...
            foreach (Material material in m_scene.Materials)
            {
                foreach (TextureSlot texSlot in material.GetAllTextures())
                {
                    textureMappings[texSlot] = textIds[index];

                    // Pridruzi teksturu odgovarajucem identifikatoru.
                    gl.BindTexture(OpenGL.GL_TEXTURE_2D, textIds[index]);
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            {
                lista.Delete(gl);
                gl.DeleteTextures(textIds.Length, textIds);
            }
        }
